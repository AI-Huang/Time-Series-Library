#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# @Date    : 2025/04/25 11:31:02
# @Author  : 黄衎
# @Email   : ex_kan.huang@tcl.com
# @Desc    : None

# Copy some lines from: https://github.com/thuml/Time-Series-Library/blob/main/run.py

import argparse
import torch
from tslib.utils.log_args import log_args
from xuntian_bigdata.backend.app.models.model_args import ModelArgs
...
def post_process_args(
    args: Union[(ModelArgs, argparse.Namespace)],
) -> argparse.Namespace:
    """对 args 进行后处理, 主要是添加 device
    Args:
        args (backend.app.models.model_args.ModelArgs): 命令行参数
    Returns:
        argparse.Namespace: 处理后的命令行参数
    注意:device 不可序列化
    """
    # 将 args 转换为 Namespace 对象
